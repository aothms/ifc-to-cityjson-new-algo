		...
		if (boost::get(boost::edge_weight, graph_, e) == edgetype_) {
			...
			if (*tgt == -1) {
				...
				if (!tgt_has_any_reflex_edge) {
					...
					// topological check completed, now check geometry

					for (size_t i = 0; i < components_.size(); ++i) {
						if (components_[i] == *src) {
							CGAL::intersection(
								graph_[i].facet->plane(),
								graph_[tgtid].facet->plane()
							);							
						}
					}
					*/
		...
		for (auto a = it->shalfedges_begin(); a != it->shalfedges_end(); ++a) {
			...
			if (a->facet()->incident_volume()->mark() && a->facet()->incident_volume() == b->facet()->incident_volume() && a->facet()->is_valid() && b->facet()->is_valid()) {
				...
				size_t aidx, bidx;
				{
					...
					aidx = p.first->second;
					if (p.second) {
						G[boost::add_vertex(G)].facet = a->facet();
					}
				}
				{
					...
					bidx = p.first->second;
					if (p.second) {
						G[boost::add_vertex(G)].facet = b->facet();
					}
				}
...
std::unique_ptr<halfspace_tree> build_halfspace_tree(Graph& G, Nef_polyhedron& poly, bool negate=false) {
	...
	std::vector<int> components(boost::num_vertices(G), -1);
	int largest_component_idx = -1;

	facet_without_reflex_edge_visitor<decltype(components)> visitor(edge_trait, components, G);
	int num_components = 0;
	for (size_t i = 0; i < boost::num_vertices(G); ++i) {
		if (components[i] == -1) {
			components[i] = num_components++;
			boost::breadth_first_search(G, boost::vertex(i, G), boost::visitor(visitor));
		}
	}

	std::ostream_iterator<size_t> output(std::cout, " ");
	std::cout << "components: ";
	std::copy(components.begin(), components.end(), output);
	std::cout << std::endl;
	...
	Filtered sub_graph_filtered(G, boost::keep_all{}, included_in_vertex_subset);
	Graph sub_graph;
